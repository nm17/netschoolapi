...
class NetSchoolAPI:
    ...
    async def login(
        self, login: str, password: str, province: str, city: str, school: str
    ):
        ...
        async with self._client as session:
            ...
            login_form = await get_login_form(
                str(session.base_url), province, city, school
            )
            ...
            student = (await session.get("webapi/student/diary/init")).json()[
                "students"
            ][0]
            self._user_id = student["studentId"]

            diary_page = await session.post(
                "/angular/school/studentdiary/", data={"AT": self._at}
            )
            self._year_id = int(
